...
int main() {
    ...
    // Добавляем первый и последний элементы в кольцо
    int extendedArr[NEW_SIZE];
    extendedArr[0] = arr[N - 1];  // добавляем последний элемент в начало массива
    for (int i = 0; i < N; i++) {
        extendedArr[i + 1] = arr[i];  // копируем исходный массив в середину расширенного массива
    }
    extendedArr[NEW_SIZE - 1] = arr[0];  // добавляем первый элемент в конец расширенного массива

    // Распечатываем полученный массив, начиная с K-ого элемента и до K-1 (по кольцу влево)
    printArray(extendedArr, K, K - 1);

    // Распечатываем полученный массив, начиная с K-ого элемента и до K+1 (по кольцу вправо)
