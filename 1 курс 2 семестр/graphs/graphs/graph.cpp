...
void Graph::SolveTSP() {
    ...
    int min_cost = numeric_limits<int>::max();  // Инициализация минимальной стоимости
    vector<int> min_route;  // Для хранения лучшего маршрута
    vector<bool> visited(n + 1, false);  // Посещенные вершины
    vector<int> path;  // Текущий путь
    path.reserve(n);
    // Функция для рекурсивного поиска маршрута
    function<void(int, int)> dfs = [&](int current, int cost) {
